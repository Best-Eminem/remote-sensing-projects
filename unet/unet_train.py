...
def train(args): 
    ...
    #model = SegNet()  
    model = unet()
    modelcheck = ModelCheckpoint(args['model'],monitor='val_acc',save_best_only=False,mode='max')  
    callable = [modelcheck]  
...
def args_parse():
    ...
    ap.add_argument("-d", "--data", help="training data's path",
                    default="./unet_train/road/")
    ap.add_argument("-m", "--model",default="./unet_buildings20.h5",
